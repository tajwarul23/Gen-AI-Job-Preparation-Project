...
const AnalyzePrepModalBody = ({ analyzeDialogRef, jobId }) => {
  ...
  //analyze mutation
  const { mutate: analyzePrep, isPending } = useAnalyzePrep();
  ...
  return (
    <dialog
      ...
      className="modal"
      onClick={(e) => {
        if (e.target === analyzeDialogRef.current) {
          closeModal();
        }
      }}
    >
      <div className="modal-box max-w-2xl bg-surface">
        ...
        <div className="flex items-center justify-end gap-2 border-t border-line p-5">
          ...
          >
            ...
            {isPending ? "Generating..." : "Generate Interview Prep"}
          </button>
        </div>
      </div>
    </dialog>
...
export default AnalyzePrepModalBody;
